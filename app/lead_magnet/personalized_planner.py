...
def generate_personalized_planner(details):
    ...
    Output should be in JSON format pure json format only. Dont include anything else like the datatype ```json and all before. just the ready to use Json! 
    
    Just follow the output JSON structure as per the below example, but content would be entirely different
    
    Example JSON structure, This example is just for 3 days, but output it for 15 days, content will be different:
    {{
    "Day 1: [Some Title 1]":
    {{
        "Action items": [
